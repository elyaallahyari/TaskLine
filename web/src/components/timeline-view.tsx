...
function rangeFor(task: Task, fallback: Date) {
  const start = task.startDate ? new Date(task.startDate) : task.dueDate ? new Date(task.dueDate) : fallback;
...
export function TimelineView({ tasks }: { tasks: Task[] }) {
  const { start, days, rows } = useMemo(() => {
    ...
    return {
      ...
      rows: dated.sort(
        (a, b) =>
          rangeFor(a, today).start.getTime() - rangeFor(b, today).start.getTime(),
      ),
    };
  }, [tasks]);
